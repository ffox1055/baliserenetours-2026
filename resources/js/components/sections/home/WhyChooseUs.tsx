...
import { whyTravelWithUsItems } from "./data";
  ...
  return (
    ...
    >
      <div className="layout py-12 sm:py-14 md:py-16 xl:px-16">
        ...
        <motion.div
          className="interactive-surface p-3 sm:p-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4 md:gap-5 xl:gap-4"
          variants={staggerContainer(!!reducedMotion, 0.1, 0.04)}
        >
          {whyTravelWithUsItems.map((item) => (
            <motion.div
              key={item.title}
              ...
              variants={staggerItem(!!reducedMotion)}
            >
              <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-15 md:h-15 bg-primary/20 rounded-full shrink-0 flex items-center justify-center">
                <item.icon
                  className="text-primary w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8"
                  strokeWidth={1.5}
                />
              </div>
